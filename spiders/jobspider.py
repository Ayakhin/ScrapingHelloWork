...
class JobSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        'DOWNLOAD_DELAY': 20  # Ajout d'un délai de 3 secondes entre chaque requête
    }
    ...
    def parse(self, response):
        ...
        # Utilisation des sélecteurs CSS pour cibler les balises span contenant les informations sur le salaire
        salary_elements = response.css('span.tw-text-jobsCandidacy::text').getall()

        # Utilisation des sélecteurs CSS ou XPath pour cibler les balises div contenant les informations sur la localisation
        ...
        for job_element, salary_info, location_element in zip(job_elements, salary_elements, location_elements):
            ...
            # Extraire le titre de l'offre d'emploi s'il existe
            if title_element:
